...
<div class="alert alert-info" role="alert"><b>Note</b>: If you are starting the calico/node container directly (and not using the <samp>calicoctl run</samp> helper command), the options are passed in environment variables. These are described in the <a href="{{site.baseurl}}/{{page.version}}/reference/node/configuration">calico/node configuration guide</a>.</div>
